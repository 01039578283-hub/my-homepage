...
def mention_items(_ctx: dict) -> list[dict]:
    return [
...
<section class="seo-geo-section" aria-labelledby="seo-geo-summary-title">
  ...
  <div class="seo-geo-grid">
    <article class="seo-geo-card"><span>지역 기준</span><strong>{html.escape(ctx['region'])} {html.escape(ctx['district'])} {html.escape(area)}</strong><p>기존 Breadcrumb와 페이지 정보를 기준으로 지역명을 명확하게 연결했습니다.</p></article>
...
def render_internal_links(page_dir: Path, ctx: dict) -> str:
    ...
    if ctx["is_child"]:
        ...
        links.append(("동네 안내", parent_title, "같은 동네의 기본 학습 안내로 이동합니다.", relative_href(page_dir, parent_dir)))
        ...
        for child in sorted([d for d in page_dir.iterdir() if d.is_dir() and (d / "index.html").exists()], key=lambda p: p.name):
            ...
            links.append(("상세 과정", ctitle, "학년별·과목별 관리 기준을 더 구체적으로 확인합니다.", relative_href(page_dir, child)))
    if not links:
        return ""
    cards = []
    ...
    org.update(
        {
            ...
            "@id": org_id,
            "name": old_org.get("name") or title,
...
def main() -> None:
    ...
    for page_dir in targets:
        ...
        if "seo-geo-enhancement:start" in current and "data-seo-geo-jsonld" in current:
            skipped += 1
            continue
        if process_page(page_dir):
            changed += 1
    sitemap_count = generate_sitemap()
    print(json.dumps({"targets": len(targets), "changed": changed, "skipped": skipped, "sitemap": sitemap_count, "date": TODAY}, ensure_ascii=False))
