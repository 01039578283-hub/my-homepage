...
def page_head(title: str, description: str, canonical_path: str, graph: list[dict[str, object]], image: str) -> str:
  ...
  <link rel="stylesheet" href="/assets/branch-directory.css?v=20260920">
...
def generate_branch_page(center: dict[str, object]) -> str:
    ...
    page_url = encoded_url(path)
    image = center["photos"][0] if center["photos"] else "/assets/title.png"
    ...
    graph.append({
        ...
        "about": {"@id": page_url + "#academy"}, "articleSection": ["센터 기본정보", "가능 과목과 학년", "인근 학교", "학습관리", "상담 안내"],
...
<section class="branch-section" id="center-info" aria-labelledby="center-info-title"><div class="branch-section-head"><p class="branch-kicker">CENTER INFORMATION</p><h2 id="center-info-title">{esc(name)} 기본정보</h2><p>센터에서 제공한 등록 정보와 운영 자료를 기준으로 정리했습니다.</p></div><dl class="info-list"><div><dt>주소</dt><dd>{esc(center["address"])}</dd></div><div><dt>등록 명칭</dt><dd>{esc(center["registeredName"])}</dd></div><div><dt>교육지원청 등록번호</dt><dd>{esc(center["registrationNumber"])}</dd></div><div><dt>평일 운영 참고</dt><dd>{esc(center["openingReference"])} · 실제 방문·수업 시간은 상담 확인</dd></div><div><dt>주말 운영</dt><dd>{esc(center["weekend"])}</dd></div><div><dt>위치 안내</dt><dd>{esc(location)}</dd></div></dl><div class="info-actions"><a class="branch-button primary" href="https://map.naver.com/p/search/{map_query}" target="_blank" rel="noopener noreferrer">네이버 지도에서 주소 검색</a><a class="branch-button" href="tel:{PHONE_LINK}">전화 상담</a></div></section>
...
def main() -> None:
    for required in (SOURCE_WORKBOOK, TARGET_WORKBOOK, PHOTO_ROOT, COMMON_PHOTO_ROOT):
